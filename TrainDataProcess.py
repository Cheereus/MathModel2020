...
from Filter import custom_filter, iir_notch_filter
...
for i in range(5):
    ...
    for j in range(12):
        ...
        for k in range(len(event)):
            ...
            if k % 13 != 0:
                # 标注
                event_label = get_true_label(char_name, event[k][0])
                event_labels.append(event_label)
                # 开始和结束时间在数据表中的索引
                event_start = event[k][1] - 1
                event_end = event_start + 200
                # 获取对应时间段的采样数据
                event_data = np.array(custom_filter(iir_notch_filter(train_data[j]))[event_start:event_end])
